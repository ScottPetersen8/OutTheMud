...
#              across Windows Events, PostgreSQL and Datadog.
...
# Initial Security section took too long so only some event IDs are being searched for.
...
when :last
  ...
  if raw =~ /\A(\d+)([hdwm])\z/i
    ...
    start_time = case unit
                 ...
                 when 'm' then now - amount*30*24*3600
                 end
    end_time = now
  else
    $logger.error "Invalid --last format. Use examples like 3m, 6h, 2d, 1w"
    exit(1)
  end
when :back
  begin
...
# -----------------------------
def collect_windows_events(log, start_time, end_time, root, verbose: false, filter_ids: nil, all_security_events: false, all_event_levels: false)
  ...
  elsif log == 'Security' && defined?(SECURITY_EVENT_IDS) && SECURITY_EVENT_IDS && !all_security_events
    $logger.info "   (Filtering Security log to #{SECURITY_EVENT_IDS.size} critical event types)"
  end
  
  if log != 'Security' && defined?(ERRORS_WARNINGS_ONLY) && ERRORS_WARNINGS_ONLY && !all_event_levels
...
begin
  ...
  unless options[:skip_windows]
    ...
    if options[:parallel]
      ...
      threads = []
      %w[System Application Security].each do |log_name|
        threads << Thread.new(log_name) do |ln|
          Thread.current.abort_on_exception = true
          collect_windows_events(ln, start_time, end_time, FOLDERS[:windows],
            verbose: options[:verbose],
            filter_ids: (ln == 'Security' ? security_ids : nil),
            all_security_events: options[:all_security_events],
            all_event_levels: options[:all_event_levels])
        end
      end
      threads.each(&:join)
    else
      $logger.info "Collecting Windows logs sequentially..."
      %w[System Application Security].each do |log_name|
        collect_windows_events(log_name, start_time, end_time, FOLDERS[:windows],
          verbose: options[:verbose],
          filter_ids: (log_name == 'Security' ? security_ids : nil),
          all_security_events: options[:all_security_events],
          all_event_levels: options[:all_event_levels])
      end
    end
